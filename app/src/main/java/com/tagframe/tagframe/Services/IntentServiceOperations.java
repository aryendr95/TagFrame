...
public class IntentServiceOperations extends IntentService implements WebServiceHandler.UploadCallbacks {
    ...
    protected void onHandleIntent(Intent intent) {
            ...
            if (frameList_models.size() > 0 && !event_id.isEmpty()) {
                for (int i = 0; i < frameList_models.size(); i++) {
                    ...
                    if (fm.getFrame_resource_type().equals(Utility.frame_resource_type_local)) {
                        Log.e("fds", fm.getStarttime() + " " + fm.getEndtime());
                        send_frames(user_id, event_id, fm.getFrametype(), fm.getName(), fm.getStarttime(),
                                fm.getEndtime(), fm.getImagepath(), fm.getProduct_id(), fm.isAProductFrame());
                    } else if (fm.isEdited()) {
                        send_edited_frames(event_id, fm.getFrame_id(), fm.getFrametype(), fm.getName(),
        ...
        } else if (operation == Constants.operation_block) {
            String user_id = intent.getStringExtra("user_id");
            String logged_user_id = intent.getStringExtra("logged_user_id");
            int block_operation = intent.getIntExtra("block_operation", 0);
            Log.e("userId", user_id);
            Log.e("luserId", logged_user_id);
            Log.e("blocked", block_operation + "");
    ...
    private void blockOp(String user_id, String logged_user_id, int block_operation) {
        ...
        apiInterface.blockUser(user_id, logged_user_id, String.valueOf(block_operation))
                    ...
                    public void onResponse(Call<ResponsePojo> call, Response<ResponsePojo> response) {
                        try {
                            if (response.body().getStatus().equalsIgnoreCase("success")) {
                                ...
                                bundle.putInt("operation", Constants.operation_block);
    ...
    private void send_edited_frames(String video_id, String frame_id, int frametype, String name,
                                    int starttime, int endtime, String imagepath, String product_id) {
        String status = "";
    ...
    private void post_event(String video_url, ArrayList<FrameList_Model> frameList_models,
                            String tittle, String descrip, int duration, String user_id, String tag) {
       ...
       /* ApiInterface apiInterface= ApiClient.getClient().create(ApiInterface.class);
        ...
        try {
            ...
            long size = tag.getBytes().length
                    + user_id.getBytes().length
                    + tittle.getBytes().length
                    + descrip.getBytes().length
                    + dur.getBytes().length
                    + event_type.getBytes().length
                    + file.length()
                    + file.getName().getBytes().length;

            WebServiceHandler webServiceHandler =
                    new WebServiceHandler(Utility.upload_video, size, WebServiceHandler.upload_video_headers);
            webServiceHandler.addFormField("user_id", user_id);
    ...
    private void send_frames(String userid, String event_id, int type, String name, int s_time,
                             int e_time, String imagepath, String product_id, boolean isproductframe) {
        ...
        if (!isproductframe) {
            ...
            size = userid.getBytes().length
                    + name.getBytes().length
                    + event_id.getBytes().length
                    + product_id.getBytes().length
                    + start_Time.getBytes().length
                    + end_Time.getBytes().length
        ...
        try {
            ...
            if (isproductframe) {
                ...
                webServiceHandler.addFilePart("media_file", file, MY_NOTIFICATION_ID,
    ...
    private void follow(final String touser_id, final String fromuser_id, int operation, String type) {
        String status = "";
        try {
...
//    StringRequest str= new StringRequest(Request.Method.POST, Utility.follow, new com.android.volley.Response.Listener<String>() {
//      @Override
//      public void onResponse(String response) {
//        try {
//          JSONObject jsonObject= new JSONObject(response);
//          JSONObject follow= jsonObject.getJSONObject("follow");
//          String status = follow.getString("status");
//          Toast.makeText(IntentServiceOperations.this, ""+status, Toast.LENGTH_SHORT).show();
//
...
//        params.put("user_id",fromuser_id);
//        params.put("to_user_id",touser_id);
//        return params;
//      }
//    };
//    requestQueue.add(str);
    ...
    private void sendNotification(int progr) {
        ...
        myNotification =
                new NotificationCompat.Builder(getApplicationContext()).setContentTitle("Uploading")
