...
public class MakeNewEvent extends Activity implements SeekBar.OnSeekBarChangeListener, Broadcastresults.Receiver {
    ...
    private ImageView ll_title_edit, img_frame_to_show, img_play_video, post_event, save_event, ll_add_frame, txt_tagged_user;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        init(savedInstanceState, event_id);
    }
    ...
    private Runnable mUpdateTimeTask = new Runnable() {
        public void run() {
            try {
                if (Utility.getScreenOrientation(MakeNewEvent.this) == ActivityInfo.SCREEN_ORIENTATION_PORTRAIT && !isLayoutSet) {
                    ...
                    if ((vidSurface.getHeight() <= height - Utility.dpToPx(150))) {
                        GridLayoutManager gridLayoutManager = new GridLayoutManager(MakeNewEvent.this, getGridColumns());
                        framelist.setLayoutManager(gridLayoutManager);
            ...
            public boolean onMenuItemClick(MenuItem item) {
                switch (item.getItemId()) {
                    ...
                    case R.id.media_library:
                        ...
                        intent.putExtra(com.veer.multiselect.Util.Constants.SELECT_TYPE, com.veer.multiselect.Util.Constants.PATH_IMAGE);
...
//            Uri imgUri = Uri.fromFile(file);
                        // selectedImageUri = ProcessnaImages.getOutputMediaFileUri(ProcessnaImages.MEDIA_TYPE_IMAGE, getApplicationContext());
...
//    }
    ...
    public File getAlbumStorageDir(Context context, String albumName) {
        ...
        File file = new File(context.getExternalFilesDir(Environment.DIRECTORY_DCIM), albumName);
        if (!file.mkdirs()) {
            Log.e("albumdir", "Directory not created");
        }
        return file;
    }
